...
public class JeuImpl implements Jeu {
    ...
    private void preparer() {
        ...
        for(i = 0;i< 4;i++) {
            ...
            Pion p = new PionImpl(this.plateau,new Position(x,y),new Position(0,0));
            this.pions.put(Couleur.values()[i], p);
            j.recevoirPion(p);
            this.joueurs.add(j);
        }
        this.objectifs = Objectif.values();
        for(i = 0; i < 12;i++) this.couloirsMobiles[i] = new CouloirMobile(Orientation.values()[r.nextInt(4)],Forme.DROIT,null,false);
        for(;i <20;i++) {
            if(i< 19) this.couloirsMobiles[i] = new CouloirMobile(Orientation.values()[r.nextInt(4)],Forme.TE,objectifs[obj++],false);
            else this.couloirsMobiles[i] = new CouloirMobile(Orientation.values()[r.nextInt(4)],Forme.TE,null,false);
        }
        for(;i <34;i++) {
            if(i< 19) this.couloirsMobiles[i] = new CouloirMobile(Orientation.values()[r.nextInt(4)],Forme.COUDE,objectifs[obj++],false);
            else this.couloirsMobiles[i] = new CouloirMobile(Orientation.values()[r.nextInt(4)],Forme.COUDE,null,false);
        }
